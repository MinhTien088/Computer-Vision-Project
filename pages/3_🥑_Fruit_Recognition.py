...
st.set_page_config(page_title="Nhận dạng trái cây", page_icon="🥑")

st.header("Nhận dạng trái cây")

try:
    if st.session_state["LoadModel3"] == True:
        print("Đã load model rồi")
except:
    model_path = os.path.join(curr_dir, "data/3_Fruit_Recognition/fruit_minh-tien.onnx")
    st.session_state["Net"] = cv2.dnn.readNet(model_path)
    st.session_state["LoadModel3"] = True
    st.success("Load model thành công!")
    print("Load model lần đầu")

filename_classes = os.path.join(curr_dir, "data/3_Fruit_Recognition/fruit_minh-tien_classes.txt")
mywidth = 640
myheight = 640
...
def postprocess(frame, outs):
    ...
    if (
        len(outNames) > 1
        or (lastLayer.type == "Region" or postprocessing == "yolov8")
        and 0 != cv2.dnn.DNN_BACKEND_OPENCV
    ):
        indices = []
...
img_file_buffer = st.file_uploader("Upload an image", type=["jpg", "jpeg", "png", "tif", "bmp"])
...
if img_file_buffer is not None:
    ...
    if st.button("Predict"):
        if not frame is None:
            ...
            if (
                st.session_state["Net"].getLayer(0).outputNameToIndex("im_info") != -1
